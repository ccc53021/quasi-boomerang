...
def get_routes_by_one_fixed_input_and_output(min_weight, max_weight, input_difference, output_difference, diff_k):
    ...
    def search_differential(f):
        ...
        # after sr
        d0_d = [btor.Var(btor.BitVecSort(state_bits), "d0_d%d" % i) for i in range(differential_characteristic_rounds_upper)]
    ...
    filename = "result/upper_bc_search_r_{}_w_{}_to_{}.txt".format(differential_characteristic_rounds_upper, min_weight, max_weight)
    f = open(filename, "w")
...
input_difference = 0x1000000000000000
output_difference = 0x0010001000000010
