...
def get_routes_by_one_fixed_input_and_output(min_weight, max_weight, input_difference, output_difference, diff_k):
    ...
    def search_differential(f):
        ...
        # difference
        # input
        d0_a = [btor.Var(btor.BitVecSort(state_bits), "d0_a%d" % i) for i in range(differential_characteristic_rounds_lower + 1)]
        # after sb
        d0_b = [btor.Var(btor.BitVecSort(state_bits), "d0_b%d" % i) for i in range(differential_characteristic_rounds_lower)]
        # after_k
        d0_c = [btor.Var(btor.BitVecSort(state_bits), "d0_c%d" % i) for i in range(differential_characteristic_rounds_lower)]
        # after sr
        d0_d = [btor.Var(btor.BitVecSort(state_bits), "d0_d%d" % i) for i in range(differential_characteristic_rounds_lower)]
        ...
        set_fixed_difference(d0_a[differential_characteristic_rounds_lower], output_difference)
        ...
        for target in range(min_weight, max_weight):
            ...
            while True:
                ...
                if r == btor.SAT:
                    ...
                    d0_a_final = int(d0_a[differential_characteristic_rounds_lower].assignment, base=2)
                    print("", file=f)
                    print("     0x{:016x},".format(d0_a_final), file=f)

                    print("    ],", file=f)
                    previous.append(([d0_a[i].assignment for i in range(differential_characteristic_rounds_lower + 1)]))
                else:
    ...
    filename = "result/lower_bc_search_r_{}_w_{}_to_{}.txt".format(differential_characteristic_rounds_lower, min_weight, max_weight)
    f = open(filename, "w")
...
input_difference = 0x0300000003000000
output_difference = 0xd0060000d0068006
...
differentials = get_routes_by_one_fixed_input_and_output(min_weight, max_weight, input_difference, output_difference, diff_route_k)
