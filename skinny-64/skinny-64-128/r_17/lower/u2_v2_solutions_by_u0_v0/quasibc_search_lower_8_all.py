...
def get_quasidifferentials_by_u0_v0(route_mask, route_diff, route_number, mask_number, min_weight, max_weight):
    ...
    filename = "result/quasibc_search_lower_{}_w_{}_to_{}_route_{}_mask_{}.txt".format(differential_characteristic_rounds_lower, min_weight, max_weight, route_number, mask_number)
    f = open(filename, "w")
...
import routes_diff as routes
diffs = routes.differentials
masks = routes.masks_by_differentials
# pool_size = 0
print(len(diffs))
all_routes = []
for routes_w in masks:
    if len(routes_w) > 0:
        print(len(routes_w))
...
for i in range(len(diffs)):
    route_diff = diffs[i]
    route_masks = masks[i]
    for j in range(len(route_masks)):
        one_mask = route_masks[j]
        pool.apply_async(get_quasidifferentials_by_u0_v0, (one_mask, route_diff, i, j, min_weight, max_weight,))
pool.close()
pool.join()