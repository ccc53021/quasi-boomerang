...
def get_quasidifferentials_by_one_fixed_input_and_output(diff_route, route_number, min_weight, max_weight):
    def search_differential(f):
        ...
        for j in range(0, state_bits, sbox_bits):
            ...
            # uu2 = u2_a[differential_characteristic_rounds_upper][j + sbox_bits - 1:j]
            ...
            # vv2 = u2_b[differential_characteristic_rounds_upper][j + sbox_bits - 1:j]
            # ww = get_one_words_weight_by_quasi_bct(u0, u1, u2, v0, v1, v2, quasi_bct)
        ...
        for i in range(differential_characteristic_rounds_upper,
            ...
            if i != differential_characteristic_rounds_upper:
                for j in range(0, state_bits, sbox_bits):
                    ...
                    # uu2 = u2_a[i][j + sbox_bits - 1:j]
                    # vv2 = u2_b[i][j + sbox_bits - 1:j]
                    delta_in_2 = (diff_route[4 * i] >> j) & (2 ** sbox_bits - 1)
                    delta_out_2 = (diff_route[4 * i + 1] >> j) & (2 ** sbox_bits - 1)
            ...
            while True:
                ...
                if r == btor.SAT:
                    ...
                    for i in range(differential_characteristic_rounds_upper + differential_characteristic_rounds_lower):
                        ...
                        if i < differential_characteristic_rounds_upper:
                            ...
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_a, diff_route[4 * i], uu1_a),
                                  file=f)
                            print("     # after sb", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_b, uu1_b, diff_route[4 * i + 1]),
                                  file=f)

                            print("", file=f)
                            print("     # lower: {}".format(int(cost_lower.assignment, base=2)), file=f)
                            print("     # after k", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_c, uu1_c, diff_route[4 * i + 2]),
                                  file=f)
                            print("     # after sr", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_d, uu1_d, diff_route[4 * i + 3]),
                                  file=f)
                            print("     # after mc", file=f)
                        else:
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_a, uu1_a, diff_route[4 * i]),
                                  file=f)
                            print("     # after sb", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_b, uu1_b, diff_route[4 * i + 1]),
                                  file=f)
                    ...
                    print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(u0_a_final, u1_a_final, diff_route[
                        4 * (differential_characteristic_rounds_upper + differential_characteristic_rounds_lower)]),
                          file=f)
