...
begin_round = 0
differential_characteristic_rounds_upper = 7
differential_characteristic_rounds_lower = 8
...
def compute_sign(route_u1, route_mask, route_diff, begin_round, round_upper, round_lower):
    correlation_sign = 0

    # upper
    for r in range(round_upper):
        u0 = route_mask[4 * r]
        delta_in_1 = route_diff[4 * r]
        u1 = route_u1[4 * r]
        v0 = route_mask[4 * r + 1]
        delta_out_1 = route_diff[4 * r + 1]
        v1 = route_u1[4 * r + 1]
    ...
    # em
    u0 = route_mask[4 * round_upper]
    delta_in_1 = route_diff[4 * round_upper]
    u1 = route_u1[4 * round_upper]
    v0 = route_mask[4 * round_upper + 1]
    v1 = route_u1[4 * round_upper + 1]
    delta_out_2 = route_diff[4 * round_upper + 1]
    for i in range(0, state_words):
        uu0 = (u0 >> (4 * i)) & (2 ** sbox_bits - 1)
        uu1 = (u1 >> (4 * i)) & (2 ** sbox_bits - 1)
        di1 = (delta_in_1 >> (4 * i)) & (2 ** sbox_bits - 1)
        ...
        c = tables.get_bct_item_by_given_all(sbox, n, m, uu0, vv0, di1, vv1, uu1, do2)
        if c < 0:
            correlation_sign += 1

    # add constants
    for r in range(begin_round, begin_round + round_upper + round_lower):
        rcs = rc_6_bits[r]
        c0 = rcs & 0xf
        c1 = (rcs >> 4) & 0x3
        c2 = 0x2
...
def get_quasidifferentials_by_u0_v0(route_mask, route_diff, route_number, min_weight, max_weight):
    def search_differential(f):
        ...
        u1_a = [btor.Var(btor.BitVecSort(state_bits), "u1_a%d" % i) for i in
                range(differential_characteristic_rounds_upper + differential_characteristic_rounds_lower + 1)]
        # after sb
        ...
        u1_b = [btor.Var(btor.BitVecSort(state_bits), "u1_b%d" % i) for i in
                range(differential_characteristic_rounds_upper + differential_characteristic_rounds_lower)]
        # after_k
        ...
        for i in range(differential_characteristic_rounds_upper):
            for j in range(0, state_bits, sbox_bits):
                ...
                delta_in_1 = (route_diff[4 * i] >> j) & (2 ** sbox_bits - 1)
                uu1 = u1_a[i][j + sbox_bits - 1:j]
                vv0 = (route_mask[4 * i + 1] >> j) & (2 ** sbox_bits - 1)
                delta_out_1 = (route_diff[4 * i + 1] >> j) & (2 ** sbox_bits - 1)
        ...
        for j in range(0, state_bits, sbox_bits):
            ...
            uu0 = (route_mask[4 * differential_characteristic_rounds_upper] >> j) & (2 ** sbox_bits - 1)
            delta_in_1 = (route_diff[4 * differential_characteristic_rounds_upper] >> j) & (2 ** sbox_bits - 1)
            uu1 = u1_a[differential_characteristic_rounds_upper][j + sbox_bits - 1:j]
            vv0 = (route_mask[4 * differential_characteristic_rounds_upper + 1] >> j) & (2 ** sbox_bits - 1)
            vv1 = u1_b[differential_characteristic_rounds_upper][j + sbox_bits - 1:j]
            delta_out_2 = (route_diff[4 * differential_characteristic_rounds_upper + 1] >> j) & (2 ** sbox_bits - 1)

            deltas = [uu0, delta_in_1, vv0, delta_out_2]
            if deltas not in count_bct:
                print("quasi bct : (0x{:01x}, 0x{:01x}) -> (0x{:01x}, 0x{:01x})".format(uu0, delta_in_1, vv0,
                                                                                        delta_out_2))
                count_bct.append(deltas)
                small_quasi_bct = tables.get_bct_matrices_2n_by_u0_v0_and_given_delta_1_and_lambda_2(sbox, n, m, uu0,
                                                                                                     vv0, delta_in_1,
                                                                                                     delta_out_2)
                small_quasi_bct_weight_lists_w_0_to_7 = get_tables_weight_lists_w_0_to_7(small_quasi_bct, dim_n, 0)
                ww = get_one_words_weight_by_small_quasi_table(uu1, vv1, small_quasi_bct_weight_lists_w_0_to_7)
                cost_em += ww
                table_bct.append(small_quasi_bct_weight_lists_w_0_to_7)
            elif deltas in count_bct:
                position = count_bct.index(deltas)
                small_quasi_bct_weight_lists_w_0_to_7 = table_bct[position]
                ww = get_one_words_weight_by_small_quasi_table(uu1, vv1, small_quasi_bct_weight_lists_w_0_to_7)
                cost_em += ww

        # the lower
        for i in range(differential_characteristic_rounds_upper,
                       differential_characteristic_rounds_upper + differential_characteristic_rounds_lower):
            if i != differential_characteristic_rounds_upper:
                for j in range(0, state_bits, sbox_bits):
                    ...
                    if deltas not in count_ddt_lower:
                        ...
                        small_quasi_ddt = tables.get_ddt_lower_matrices_2n_by_u0_v0_and_given_delta_2_and_lambda_2(sbox,
                                                                                                                   n, m,
                                                                                                                   uu0,
                                                                                                                   vv0,
                                                                                                                   delta_in_2,
                                                                                                                   delta_out_2)
                        small_quasi_ddt_weight_lists_w_0_to_7 = get_tables_weight_lists_w_0_to_7(small_quasi_ddt, dim_n,
                                                                                                 1)
                        w = get_one_words_weight_by_small_quasi_table(uu1, vv1, small_quasi_ddt_weight_lists_w_0_to_7)
                        # cost_lower += w
                        cost_lower += w
        ...
        set_fixed_difference(u1_a[differential_characteristic_rounds_upper + differential_characteristic_rounds_lower],
                             0x0)
                ...
                if r == btor.SAT:
                    ...
                    for i in range(differential_characteristic_rounds_upper + differential_characteristic_rounds_lower):
                        ...
                        if i < differential_characteristic_rounds_upper:
                            ...
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(route_mask[4 * i], route_diff[4 * i],
                                                                                   uu1_a), file=f)
                            print("     # after sb", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(route_mask[4 * i + 1],
                                                                                   route_diff[4 * i + 1], uu1_b),
                                  file=f)
                            print("     # after k", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(route_mask[4 * i + 2],
                                                                                   route_diff[4 * i + 2], uu1_c),
                                  file=f)
                            print("     # after sr", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(route_mask[4 * i + 3],
                                                                                   route_diff[4 * i + 3], uu1_d),
                                  file=f)
                    ...
                    u1_a_final = int(u1_a[
                                         differential_characteristic_rounds_upper + differential_characteristic_rounds_lower].assignment,
                                     base=2)
                    # u2_a_final = int(u2_a[differential_characteristic_rounds_upper + differential_characteristic_rounds_lower].assignment, base=2)
                    print("", file=f)
                    print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(route_mask[4 * (
                                differential_characteristic_rounds_upper + differential_characteristic_rounds_lower)],
                                                                           u1_a_final, route_diff[4 * (
                                    differential_characteristic_rounds_upper + differential_characteristic_rounds_lower)]),
                          file=f)
                    ...
                    s_route = compute_sign(route_u1, route_mask, route_diff, begin_round, differential_characteristic_rounds_upper,
                                           differential_characteristic_rounds_lower)
                    if s_route < 0:
                        count_negative += 1
                        # print("    # sign = -1,", file=f)
                    else:
                    ...
                    previous.append((s_route, [u1_a[i].assignment for i in range(
                        differential_characteristic_rounds_upper + differential_characteristic_rounds_lower + 1)]))
                else:
    ...
    filename = "result/quasibc_search_upper_{}_lower_{}_w_{}_to_{}_route_{}.txt".format(differential_characteristic_rounds_upper, differential_characteristic_rounds_lower, min_weight, max_weight, route_number)
    f = open(filename, "w")
