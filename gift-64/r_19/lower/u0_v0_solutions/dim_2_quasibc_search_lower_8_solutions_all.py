...
def compute_sign(route_u0, route_u1, diff_route, begin_round, round_lower):
    correlation_sign = 0
...
def get_quasidifferentials_by_one_fixed_input_and_output(diff_route, route_number, min_weight, max_weight):
    def search_differential(f):
        ...
        set_fixed_difference(u0_a[differential_characteristic_rounds_lower],
                             0x0)
        set_fixed_difference(u1_a[differential_characteristic_rounds_lower],
                             0x0)
            ...
            while True:
                ...
                if r == btor.SAT:
                    ...
                    for i in range(differential_characteristic_rounds_lower):
                        ...
                        if i < differential_characteristic_rounds_lower:
                            if i == 0:
                    ...
                    u0_a_final = int(u0_a[differential_characteristic_rounds_lower].assignment,
                                     base=2)
                    u1_a_final = int(u1_a[differential_characteristic_rounds_lower].assignment,
                                     base=2)
                    print("", file=f)
                    print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(u0_a_final, u1_a_final, diff_route[
                        3 * (differential_characteristic_rounds_lower)]),
                          file=f)
                    route_u0.append(u0_a_final)
                    route_u1.append(u1_a_final)

                    s_route = compute_sign(route_u0, route_u1, diff_route, begin_round, differential_characteristic_rounds_lower)
                    if s_route < 0:
                        count_negative += 1
                    else:
                        count_positive += 1
                    ...
                    print("    ],", file=f)
                    previous.append((s_route, [u0_a[i].assignment for i in range(differential_characteristic_rounds_lower + 1)]))
                else:
    ...
    filename = "result/dim_2_solutions_lower_{}_w_{}_to_{}_route_{}.txt".format(
        differential_characteristic_rounds_lower, min_weight, max_weight,
        route_number)
    f = open(filename, "w")
