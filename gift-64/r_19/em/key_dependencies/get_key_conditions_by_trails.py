...
def count_valid_by_given_basis(key_list1, basis, basis_values):
    key_0 = [0] * (key_bits * 3 + 1)
    key_1 = [0] * (key_bits * 3 + 1)
    key_1[key_bits * 3] = 1
    ...
    for i in range(len(key_list1)):
        ...
        if one_key == key_0:
            ...
            for j in range(len(basis)):
                ...
                #         break
                pp_k_list = one_basis
                one_basis_k_x = pp_k_list[0]
                one_basis_k_upper = pp_k_list[1]
                one_basis_k_lower = pp_k_list[2]
                for one_key_pp in one_basis_k_x:
                    if one_key_pp not in one_key_list:
                        flag_y = 0
                        break
                for one_key_pp in one_basis_k_upper:
                    new_position = one_key_pp[0] * 16 + one_key_pp[1] + key_bits
                    if [new_position // 16, new_position % 16] not in one_key_list:
                        flag_y = 0
                        break
                for one_key_pp in one_basis_k_lower:
                    new_position = one_key_pp[0] * 16 + one_key_pp[1] + key_bits * 2
                    if [new_position // 16, new_position % 16] not in one_key_list:
                        flag_y = 0
                        break
...
for i in range(2 ** 0):
    ...
    for ww in range(max_w):
        if len(key_list1_with_w[ww]) > 0:
            ...
            print("w = {} : {} quasi-bcs, {} quasi-bcs in fixed-key, p_w = 2^-{} * {}".format(ww, count_w_and_numbers[ww], count_w_and_numbers_in_fixed_key[ww], ww, count_w_and_numbers_in_fixed_key[ww]))
