...
def compute_sign(route_u0, route_u1, diff_route, round_upper, round_lower):
    ...
    # add constants
    for r in range(round_upper + round_lower):
...
def get_quasidifferentials_by_one_fixed_input_and_output(diff_route, route_number, min_weight, max_weight):
    def search_differential(f):
        ...
        for i in range(differential_characteristic_rounds_upper):
            for j in range(0, state_bits, sbox_bits):
                ...
                delta_in_1 = (diff_route[3 * i] >> j) & (2 ** sbox_bits - 1)
                delta_out_1 = (diff_route[3 * i + 1] >> j) & (2 ** sbox_bits - 1)
        ...
        for j in range(0, state_bits, sbox_bits):
            ...
            delta_in_1 = (diff_route[3 * differential_characteristic_rounds_upper] >> j) & (2 ** sbox_bits - 1)
            delta_out_2 = (diff_route[3 * differential_characteristic_rounds_upper + 1] >> j) & (2 ** sbox_bits - 1)
            ...
            deltas = [delta_in_1, delta_out_2]
            if deltas not in count_bct:
                print("0x{:01x} -> 0x{:01x}".format(delta_in_1, delta_out_2))
                count_bct.append(deltas)
                small_quasi_bct = tables.get_bct_matrices_2n_by_given_delta_1_and_lambda_2(sbox, n, m, delta_in_1,
                                                                                           delta_out_2)
                small_quasi_bct_weight_lists_w_0_to_7 = get_tables_weight_lists_w_0_to_7(small_quasi_bct, dim_n, 0)
                ww = get_one_words_weight_by_small_quasi_table(uu0, uu1, vv0, vv1,
                                                               small_quasi_bct_weight_lists_w_0_to_7)
                cost_em += ww
        ...
        for i in range(differential_characteristic_rounds_upper,
            ...
            if i != differential_characteristic_rounds_upper:
                for j in range(0, state_bits, sbox_bits):
                    ...
                    uu1 = u1_a[i][j + sbox_bits - 1:j]
                    vv1 = u1_b[i][j + sbox_bits - 1:j]
                    delta_in_2 = (diff_route[3 * i] >> j) & (2 ** sbox_bits - 1)
                    delta_out_2 = (diff_route[3 * i + 1] >> j) & (2 ** sbox_bits - 1)

                    deltas = [delta_in_2, delta_out_2]
                    if deltas not in count_ddt_lower:
                        print("0x{:01x} -> 0x{:01x}".format(delta_in_2, delta_out_2))
                        count_ddt_lower.append(deltas)
                        small_quasi_ddt = tables.get_ddt_lower_matrices_2n_by_given_delta_2_and_lambda_2(sbox, n, m,
                                                                                                         delta_in_2,
                                                                                                         delta_out_2)
        ...
        set_fixed_difference(u0_a[differential_characteristic_rounds_upper + differential_characteristic_rounds_lower],
                             0x0)
                ...
                if r == btor.SAT:
                    ...
                    for i in range(differential_characteristic_rounds_upper + differential_characteristic_rounds_lower):
                        ...
                        if i < differential_characteristic_rounds_upper:
                            ...
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_a, diff_route[3 * i], uu1_a),
                                  file=f)
                            print("     # after sb", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_b, diff_route[3 * i + 1], uu1_b),
                                  file=f)
                            print("     # after p", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_c, diff_route[3 * i + 2], uu1_c),
                                  file=f)
                            ...
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_a, diff_route[3 * i], uu1_a),
                                  file=f)
                            print("     # after sb", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_b, uu1_b, diff_route[3 * i + 1]),
                                  file=f)

                            print("", file=f)
                            print("     # lower: {}".format(int(cost_lower.assignment, base=2)), file=f)
                            print("     # after p", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_c, uu1_c, diff_route[3 * i + 2]),
                                  file=f)
                            print("     # after k", file=f)
                        else:
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_a, uu1_a, diff_route[3 * i]),
                                  file=f)
                            print("     # after sb", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(uu0_b, uu1_b, diff_route[3 * i + 1]),
                                  file=f)
                    ...
                    u1_a_final = int(u1_a[
                                     ...
                                     base=2)
                    print("", file=f)
                    print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(u0_a_final, u1_a_final, diff_route[
                        3 * (differential_characteristic_rounds_upper + differential_characteristic_rounds_lower)]),
                          file=f)
                    ...
                    print("    ],", file=f)
                    previous.append((s_route, [u0_a[i].assignment for i in range(differential_characteristic_rounds_upper + differential_characteristic_rounds_lower + 1)]))
                else:
    ...
    filename = "result/dim_2_solutions_upper_{}_lower_{}_w_{}_to_{}_route_{}.txt".format(
        differential_characteristic_rounds_upper, differential_characteristic_rounds_lower, min_weight, max_weight,
        route_number)
    f = open(filename, "w")
