...
def compute_sign(route_u1, route_mask, route_diff, begin_round, round_upper, round_lower):
    ...
    for r in range(round_upper + 1, round_upper + round_lower):
        u0 = route_mask[3 * r]
        u1 = route_u1[3 * r]
        delta_in_2 = route_diff[3 * r]
        v0 = route_mask[3 * r + 1]
        v1 = route_u1[3 * r + 1]
        delta_out_2 = route_diff[3 * r + 1]
    ...
    # em
    u0 = route_mask[3 * round_upper]
    delta_in_1 = route_diff[3 * round_upper]
    u1 = route_u1[3 * round_upper]
    v0 = route_mask[3 * round_upper + 1]
    v1 = route_u1[3 * round_upper + 1]
    delta_out_2 = route_diff[3 * round_upper + 1]
    for i in range(0, state_words):
        uu0 = (u0 >> (4 * i)) & (2 ** sbox_bits - 1)
        uu1 = (u1 >> (4 * i)) & (2 ** sbox_bits - 1)
        di1 = (delta_in_1 >> (4 * i)) & (2 ** sbox_bits - 1)
    ...
    for r in range(begin_round, begin_round + round_upper + round_lower):
        ...
        v0 = route_mask[3 * (r - begin_round) + 1]
...
def get_quasidifferentials_by_u0_v0(route_mask, route_diff, route_number, min_weight, max_weight):
    def search_differential(f):
        ...
        for j in range(0, state_bits, sbox_bits):
            ...
            uu0 = (route_mask[3 * differential_characteristic_rounds_upper] >> j) & (2 ** sbox_bits - 1)
            delta_in_1 = (route_diff[3 * differential_characteristic_rounds_upper] >> j) & (2 ** sbox_bits - 1)
            uu1 = u1_a[differential_characteristic_rounds_upper][j + sbox_bits - 1:j]
            vv0 = (route_mask[3 * differential_characteristic_rounds_upper + 1] >> j) & (2 ** sbox_bits - 1)
            vv1 = u1_b[differential_characteristic_rounds_upper][j + sbox_bits - 1:j]
            delta_out_2 = (route_diff[3 * differential_characteristic_rounds_upper + 1] >> j) & (2 ** sbox_bits - 1)
        ...
        for i in range(differential_characteristic_rounds_upper,
            ...
            if i != differential_characteristic_rounds_upper:
                for j in range(0, state_bits, sbox_bits):
                    uu0 = (route_mask[3 * i] >> j) & (2 ** sbox_bits - 1)
                    uu1 = u1_a[i][j + sbox_bits - 1:j]
                    delta_in_2 = (route_diff[3 * i] >> j) & (2 ** sbox_bits - 1)
                    vv0 = (route_mask[3 * i + 1] >> j) & (2 ** sbox_bits - 1)
                    vv1 = u1_b[i][j + sbox_bits - 1:j]
                    delta_out_2 = (route_diff[3 * i + 1] >> j) & (2 ** sbox_bits - 1)
                    ...
                    for i in range(differential_characteristic_rounds_upper + differential_characteristic_rounds_lower):
                        ...
                        if i < differential_characteristic_rounds_upper:
                            ...
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(route_mask[3 * i], route_diff[3 * i],
                                                                                   uu1_a), file=f)
                            print("     # after sb", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(route_mask[3 * i + 1], uu1_b,
                                                                                   route_diff[3 * i + 1]), file=f)

                            print("", file=f)
                            print("     # lower: {}".format(int(cost_lower.assignment, base=2)), file=f)
                            print("     # after p", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(route_mask[3 * i + 2], uu1_c,
                                                                                   route_diff[3 * i + 2]), file=f)
                            print("     # after k", file=f)
                        else:
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(route_mask[3 * i], uu1_a,
                                                                                   route_diff[3 * i]), file=f)
                            print("     # after sb", file=f)
                            print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(route_mask[3 * i + 1], uu1_b,
                                                                                   route_diff[3 * i + 1]), file=f)
                            print("     # after p", file=f)
